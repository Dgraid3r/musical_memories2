...
def create_signin_state(nonce: str) -> str:
    """A short-lived, signed token protecting the Google sign-in redirect
    ...
    `nonce` is generated by the caller (routers/google_auth.py's login())
    and also placed in an httpOnly cookie on the same response - embedding
    it here rather than generating it internally lets verify_signin_state
    below compare the two and confirm this callback is answering a request
    issued to *this specific browser*, not just that the server issued
    *some* valid google_signin state recently (see verify_signin_state's
    docstring for why that distinction matters)."""
    expire = datetime.now(timezone.utc) + timedelta(minutes=OAUTH_STATE_EXPIRE_MINUTES)
    payload = {"purpose": "google_signin", "nonce": nonce, "exp": expire}
    return jwt.encode(payload, _secret_key(), algorithm=ALGORITHM)


def verify_signin_state(state: str, cookie_nonce: str | None) -> bool:
    """True only if `state` is a still-valid, untampered token from
    create_signin_state AND its embedded nonce matches `cookie_nonce` - the
    value of the httpOnly cookie login() set on the same browser that
    started this sign-in attempt.

    The signature/expiry/purpose checks alone only prove the server issued
    *some* valid google_signin state recently - they don't prove it issued
    *this* one to *this* browser. Without the cookie comparison, an
    attacker could start their own sign-in to obtain a validly-signed
    state + a matching authorization code, then get a victim's browser to
    hit this callback with the attacker's code and that state; the
    signature would check out and the victim would end up logged into the
    attacker's account. Comparing the state's nonce against a cookie only
    the browser that requested that exact state ever received closes that
    hole - an attacker's own state/code pair is worthless without also
    controlling the victim's cookie jar.

    Returns False (never raises) for a missing cookie, a missing/expired/
    tampered/wrong-purpose state (including, deliberately, some other
    valid JWT this server issued for a different purpose, like an access
    token or a Spotify oauth_state), or a nonce mismatch."""
    if not cookie_nonce:
        return False
    try:
        payload = jwt.decode(state, _secret_key(), algorithms=[ALGORITHM])
    ...
    if payload.get("purpose") != "google_signin":
        return False
    return secrets.compare_digest(payload.get("nonce") or "", cookie_nonce)
